...
class Block(nn.Module):
    def __init__(self, w_in, w_out, stride, group_width, bottleneck_ratio, se_ratio, is_last=False):
...
class RegNet(nn.Module):
    ...
    def _make_layer(self, idx):
        ...
        for i in range(depth):
            ...
            layers.append(Block(self.in_planes, width,
                                s, group_width, bottleneck_ratio, se_ratio, is_last=(i == depth-1)))
    ...
    def distill_seq(self):
        feat_m = nn.ModuleList([])  
        #feat_m.append(nn.Sequential(self.conv1, self.bn1, self.relu))       
        feat_m.append(self.layer2)
        feat_m.append(self.layer3)
        feat_m.append(self.layer4)
    ...
    # def forward(self, x):
    #     out = x = self.relu(self.bn1(self.conv1(x)))
    #     out = self.layer1(out)
    #     out = self.layer2(out)
    #     out = self.layer3(out)
    #     out = self.layer4(out)
...
if __name__ == '__main__':
    ...
    net = RegNetY_400MF(num_classes=100)
    logit = net(x)
